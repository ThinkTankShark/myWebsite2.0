...
(function() {
  window["cama_init_media"] = function(media_panel) {
    ...
    show_file = function(item) {
      var _hh, _ww, btn, cut, data, hh, tpl, ww;
      item.addClass('selected').siblings().removeClass('selected');
      data = eval("(" + item.find(".data_value").val() + ")");
      media_info_tab_info.click();
      tpl = "<div class='p_thumb'></div>" + "<div class='p_label'><b>" + I18n("button.name") + ": </b><br> <span>" + data["name"] + "</span></div>" + "<div class='p_body'>" + "<div><b>" + I18n("button.url") + ":</b><br> <a target='_blank' href='" + data["url"] + "'>" + data["url"] + "</a></div>" + "<div><b>" + I18n("button.size") + ":</b> <span>" + cama_humanFileSize(data["size"]) + "</span></div>" + "</div>";
    ...
    customFileData = function() {
      return {
        folder: media_panel.attr("data-folder").replace(/\/{2,}/g, '/'),
    ...
    media_panel.find('.cam_media_reload').click(function(e) {
      showLoading();
      $.get(media_panel.attr("data-url"), {
        partial: true,
        media_formats: media_panel.attr("data-formats"),
      ...
      callback = function(modal) {
        ...
        return modal.find("form").submit(function() {
          ...
          $.post(media_panel.attr("data-url_actions"), {
            folder: media_panel.attr("data-folder") + "/" + input.val().replace(/\/{2,}/g, '/'),
    ...
    media_panel.on("click", "a.del_item, a.del_folder", function() {
      ...
      showLoading();
      $.post(media_panel.attr("data-url_actions"), {
        folder: media_panel.attr("data-folder") + "/" + item.attr("data-key").replace(/\/{2,}/g, '/'),
